...
# User controls for specific 3D snapshot hour selection
months_ordered = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
t_col1, t_col2 = st.columns(2)
with t_col1:
    select_m = st.selectbox("Select Preview Month", months_ordered, index=5) # Default June
with t_col2:
    select_h = st.slider("Select Preview Hour (24h)", 0, 23, 12)

# Filter out the matching row from our precalculated annual dataset
...
if not filtered_df.empty:
    ...
    # Shadow (Rendered dynamically if available at selected hour)
    if preview_shadow:
    ...
    st.warning("No data found for this specific timestamp selection.")
