...
# 4. LIVE Interactive 3D VISUALIZER WITH TIME TIME CONTROLS
...
if not filtered_df.empty:
    ...
    preview_shadow = None
    # FIXED: Clip shadow boundary to the wall limits (-4 to 4, 0 to 4) instead of just the window box
    if p_alt > 0 and p_sun[1] >= 0.001:
        ...
        # Intersect shadow with the entire wall area so it renders on the siding too
    ...
    # Shadow (Now renders completely over the wall face)
    if preview_shadow:
        fig3d.add_trace(go.Scatter3d(x=[pt[0] for pt in preview_shadow], y=[-0.02]*len(preview_shadow), z=[pt[1] for pt in preview_shadow], mode='lines', surfaceaxis=1, surfacecolor='rgba(40, 45, 55, 0.6)', name="Shadow"))
