...
if not filtered_df.empty:
    ...
    if p_alt > 0 and p_sun[1] >= 0.001:
        ...
        p_shapes = [pt + (-pt[1] / p_sun[1]) * p_sun for pt in [c1, c2, c3, c4]]
    ...
    # Shadow (Shifted slightly forward to y = -0.02 to avoid z-fighting texture overlaps)
    if preview_shadow:
    ...
    fig3d.update_layout(scene=dict(xaxis_range=[-4, 4], yaxis_range=[-3, 3], zaxis_range=[0, 4], aspectmode='manual', aspectratio=dict(x=1, y=0.7, z=0.5), camera=dict(eye=dict(x=1.5, y=-2.2, z=1.5))), margin=dict(l=0, r=0, b=0