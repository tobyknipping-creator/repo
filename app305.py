...
from pvlib.irradiance import get_extra_radiation, clear_sky__ineichen
...
# 180 is South. Let's provide an intuitive helper label next to the slider
...
# Intuitive cardinal direction label mapping
...
# 3. CORE MATH: ANNUAL SIMULATION ENGINE (WITH ROTATION MATRIX)
...
def run_annual_simulation(lat, lon, w_width, w_height, c_width, c_depth, shgc_val, wall_ori):
    ...
    ext_rad = get_extra_radiation(times)
    cs_irrad = clear_sky__ineichen(solpos['apparent_zenith'], solpos['apparent_elevation'], ext_rad)
    ...
    for i in range(len(times)):
        ...
        # FIXED ROTATION MATH: Shift the sun's azimuth relative to the custom wall orientation face
        ...
        # Calculate real component of radiation striking an inclined vertical profile
        # Cosine law factor of incidence angle to the wall surface
        cos_incidence = np.cos(alt_rad) * np.cos(az_rad)
        if cos_incidence < 0: cos_incidence = 0 # Sun is casting light from behind the wall structure
...
# Calculate a single visual mock preview state for June 21st at 12:00 PM matching the wall rotation
preview_idx = 4116 # Hour index for June 21st at 12:00 PM
...
# Establish physical geometry paths
